...
class Cart extends BaseModel
{
    ...
    {
        $stmt = $this->db->prepare('SELECT ci.*, p.name AS product_name, p.slug AS product_slug, pv.title AS variation_title FROM cart_items ci JOIN products p ON p.id = ci.product_id JOIN product_variations pv ON pv.id = ci.variation_id WHERE ci.cart_id = :cart_id');
