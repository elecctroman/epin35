...
class Order extends BaseModel
{
    ...
    public function licenseVault(int $userId, int $limit = 20): array
    {
        $stmt = $this->db->prepare('SELECT oi.delivered_payload, oi.qty, o.id AS order_id, o.created_at, p.name, pv.title
            FROM order_items oi
            JOIN orders o ON o.id = oi.order_id
            JOIN products p ON p.id = oi.product_id
            JOIN product_variations pv ON pv.id = oi.variation_id
            WHERE o.user_id = :user_id AND oi.delivered_payload IS NOT NULL
